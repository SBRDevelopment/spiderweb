...
# Modified haproxy-marathon-bridge script by Brian Wight
# https://github.com/mesosphere/marathon/blob/master/bin/haproxy-marathon-bridge

set -o errexit -o nounset -o pipefail
function -h {
cat <<USAGE
 USAGE: $name <spiderweb host:port>+
        $name install_haproxy_system <spiderweb host:port>+
...
cronjob_conf_file=/etc/"$name"/spiderweb
...
function install_haproxy_system {
  ...
  then 
    sudo env DEBIAN_FRONTEND=noninteractive apt-get install -y haproxy
...
function install_cronjob {
  ...
  script "$@" | sudo dd of="$script_path"
  sudo chmod ug+rx "$script_path"
  cronjob  | sudo dd of="$cronjob"
  config "$@" | sudo dd of=/etc/haproxy/"$conf_file"
}
...
function config {
  curl -sSfLk -H 'Accept: text/plain' "${1%/}"/api/v1.0/template/default/render.txt
}

function script {
  curl -sSfLk -H 'Accept: text/plain' "${1%/}"/install-crontab.sh
}
